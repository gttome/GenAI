...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentBookMetadata = { title: "Generative AI Prompt Guide Reader", author: "Unknown Author", artwork: [{ src: 'images/default-cover-512.png', sizes: '512x512', type: 'image/png' }] }; 
    ...
    function displayUserMessage(message, type = 'info') {
        ...
        metadataStatusDisplay.className = `status-message status-${type}`; // For potential styling based on type
        // Auto-clear message after a few seconds for non-error messages
        if (type !== 'error') {
    ...
    function saveCurrentChapterProgress() {
        if (currentChapterIndex !== -1 && chapters[currentChapterIndex] && audioPlayer.currentTime > 0) {
            const chapterFileName = chapters[currentChapterIndex].fileName;
    ...
    async function loadChapters() { 
        ...
        try {
            ...
            const errorMsg = `Error loading chapters: ${error.message}. Ensure 'chapters-manifest.json' is correct.`;
    ...
    async function selectChapterAndPlay(index, playFromTimestamp = 0) { 
        ...
        if (playFromTimestamp === 0) { // Only check per-chapter progress if not explicitly told where to start (e.g. from bookmark or global resume)
        ...
        // Using 'loadedmetadata' might be more reliable for setting currentTime before play,
        // as 'canplay' might fire too late if the browser already started playing a bit.
        audioPlayer.addEventListener('loadedmetadata', setTimeAndPlay, {once: true}); // Use once to ensure it only fires for this load
        audioPlayer.addEventListener('canplay', setTimeAndPlay, {once: true}); // Fallback or alternative timing

        currentChapterTitleDisplay.textContent = chapter.title;
        updateChapterListActiveState();
        await fetchAndDisplayMetadata(chapter, false, false); 
        saveGlobalLastPosition(); 
    }
    ...
    async function fetchAndDisplayMetadata(chapter, isBookLevel = false, dontAutoPlay = false) { 
        ...
        if (!isBookLevel) { coverArtImg.src = currentBookMetadata.artwork[0].src; coverArtImg.alt = "Loading cover art..."; }
        try {
            let extracted = { 
                title: chapter.title, 
                ...
                coverArtBlob: null, 
                artworkSrc: currentBookMetadata.artwork[0].src 
            };
            // console.warn("M4B parsing library not yet implemented for metadata extraction.");
            
            if (isBookLevel) {
                currentBookMetadata.title = extracted.album || "Generative AI Prompt Guide Reader"; 
            ...
            coverArtImg.alt = `${extracted.title || 'Audiobook'} Cover Art`;
            ...
            coverArtImg.src = currentBookMetadata.artwork[0].src; 
    ...
    function updateMediaSessionMetadata(chapterTitle, author, bookTitle, artworkSrcInput) {
        if ('mediaSession' in navigator) {
            const artworkPath = artworkSrcInput || currentBookMetadata.artwork[0].src || 'images/default-cover-512.png'; // Ensure default path
            navigator.mediaSession.metadata = new MediaMetadata({
                title: chapterTitle || 'Unknown Chapter', artist: author || 'Unknown Author',
    ...
    function setupMediaSessionActions() {
        if ('mediaSession' in navigator) {
            ...
            navigator.mediaSession.setActionHandler('nexttrack', () => { 
                if (currentChapterIndex < chapters.length - 1) selectChapterAndPlay(currentChapterIndex + 1); 
            });
    ...
    audioPlayer.addEventListener('loadedmetadata', () => { 
        ...
        if (chapters[currentChapterIndex]) {
            fetchAndDisplayMetadata(chapters[currentChapterIndex], false, false); 
        }
    });
    ...
    // Initialize book title and author from metadata defaults
    currentBookTitleDisplay.textContent = currentBookMetadata.title; 
    currentBookAuthorDisplay.textContent = currentBookMetadata.author; 
});
