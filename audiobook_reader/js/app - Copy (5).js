...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadAppContent() { 
        ...
        try {
            ...
            const detailsFromFile = await bookDetailsResponse.json(); // Renamed to avoid conflict
            ...
            // Populate global book details more robustly
            bookDetails.bookTitle = detailsFromFile.bookTitle || bookDetails.bookTitle; // Use default if not in JSON
            if (detailsFromFile.hasOwnProperty('bookAuthor')) { // Check if key exists
                bookDetails.bookAuthor = detailsFromFile.bookAuthor;
            } else {
                console.warn("book_details.json is missing 'bookAuthor'. Using default.");
                // bookDetails.bookAuthor remains the initialized default
            }
            ...
            console.log("Loaded book details:", bookDetails);
    ...
    async function fetchAndDisplayMetadata(chapter) { 
        ...
        currentBookTitleDisplay.textContent = bookDetails.bookTitle;
        currentBookAuthorDisplay.textContent = bookDetails.bookAuthor; // Use the global book author
        ...
        if (savedPosition) {
            try {
                ...
                if (chapterIdx !== -1 && lastPosition.timestamp) {
                    console.log("Resuming from global last position:", `Chapter ${chapterIdx + 1}`, `at ${formatTime(lastPosition.timestamp)}`);
