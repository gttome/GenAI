...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndDisplayMetadata(chapter, isBookLevel = false, dontAutoPlay = false) { 
        ...
        try {
            ...
            // console.warn("M4B parsing library not yet implemented for metadata extraction."); // Keep this reminder
        ...
        if (savedPosition) {
            try {
                ...
                if (chapterExists && lastPosition.chapterIndex >= 0 && lastPosition.chapterIndex < chapters.length) {
                    // console.log("Resuming from global last position:", `Chapter ${lastPosition.chapterIndex + 1}`, `at ${formatTime(lastPosition.timestamp)}`); // Less critical
    ...
    function updateMediaSessionMetadata(chapterTitle, author, bookTitle, artworkSrcInput) {
        if ('mediaSession' in navigator) {
            const artworkPath = artworkSrcInput || currentBookMetadata.artwork[0].src || 'images/default-cover-512.png';
            ...
            // console.log("Media session metadata updated."); // Can be noisy
        }
    }
    function updateMediaSessionState() {
        if ('mediaSession' in navigator) {
            navigator.mediaSession.playbackState = (audioPlayer.paused || audioPlayer.ended) ? 'paused' : 'playing';
            // console.log("Media session playbackState updated to:", navigator.mediaSession.playbackState); // Can be noisy
        }
    }
    function setupMediaSessionActions() {
        if ('mediaSession' in navigator) {
            ...
            console.log("Media Session action handlers set up."); // Keep this one-time setup log
        }
    }
